// File: testexple_mpi_stmm.c, version 1.1b, July 2012  
//
// Provide using examples for the symmetric Toeplitz matrix-matrix product routine (mpi_stmm).
// 
...
//still contains the bigger data size due to the remainder of the euclidean division.
  if(test==3) {
    ...
    if (size<5){
      printf("Sorry, you need to use 5 MPI processes at least and you used %d only.\n\n", size);
      MPI_Finalize();
      exit(1);}
...
//  flag_stgy_init_defined(&flag_stgy);
  ...
  mpi_stmm(&Vrank, n, m, displs[rank], nranks[rank], T, lambda, flag_stgy, MPI_COMM_WORLD);
  t2=  MPI_Wtime();

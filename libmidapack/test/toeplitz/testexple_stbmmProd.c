//
// File: testexple_stbmmProd.c, version 1.1b, July 2012  
//
// Provide using examples for the symmetric Toeplitz block-diagonal matrix-matrix product
// routine using the wizard stbmmProd routine (using strutures) instead of the mpi_stbmm
// routine.
...
//Convert to Tpltz structure:
 ...
 Nm1.nb_blocks_loc = nb_blocks;
 Nm1.nb_blocks_tot = nb_blocks;
 Nm1.idp = id0;
 Nm1.local_V_size = local_V_size;
 Nm1.flag_stgy = flag_stgy;
 Nm1.comm = MPI_COMM_WORLD;
...
// use stbmmProd instead of mpi_stbmm
  stbmmProd( Nm1 , Vrank);

...
int main(int argc, char *argv[]){
  ...
  if( !rank) printf(" scan ok \n");
  
  MPI_Barrier( MPI_COMM_WORLD);

  MatSetIndices(&A, nt, 1, pix);

  if( !rank) printf(" indices ok : %ld \n", nt*sizeof(double));
  ...
  y1 = (double *) malloc((int64_t) nt* sizeof(double));          //< allocate vector values  according partitioning ant init to 0.0
  ...
  /* rs 2015/02/04 just for output tests ... */
  char fname[64];
  FILE *fout;

  sprintf( fname, "test_rank%d_flag%d.bin",rank, flag);
  fout = fopen( fname, "w"); fwrite( x1, A.lcount, sizeof( double), fout); fclose( fout);

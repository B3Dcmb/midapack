...
void usage(){ 
    printf("\n*#  Usage: mpirun -n [number of processes] test_I_mapmat [option[value]] ");
...
int main(int argc, char *argv[]){
  ...
  /*set options*/
  while((ch = getopt( argc, argv, "M:f:s:o:ti" )) != EOF){
    switch(ch) {
      case 'M':
  ...
  MatSetValues(&A, m, 0, 1, values);
  MatSetIndices(&A, m, 0, 1, indices);

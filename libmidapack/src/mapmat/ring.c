...
/** @brief Initialize tables for ring-like communication scheme.
    ...
    Sending and receiving tabs should be well allocated(at least size of number of steps in ring scheme).
    Double pointer are partially allocated, the last allocation is performed inside the routine (only for R S are just pointer).
...
/** @brief Perform a sparse sum reduction (or mapped reduction) using a ring-like communication scheme
...
/** @brief Perform a sparse sum reduction (or mapped reduction) using an MPI-Alltoallv call
...
int alltoallv_reduce(int **R, int *nR, int nRtot, int **S, int *nS, int nStot, double *val, double *res_val, int steps, MPI_Comm comm){
	...
	MPI_Comm_size(comm, &size);  // N.B. size and steps must be equal, shall we check for this ?! -- rs
	MPI_Comm_rank(comm, &rank);
	...
	MPI_Alltoallv(sbuf,sindx,sdisp,MPI_DOUBLE,rbuf,rindx,rdisp,MPI_DOUBLE,comm);

	// accumulate contributions ...

	for (p=0; p<steps; p++){
		rp=(rank+size-p)%size;
		s2m_sum(res_val, &rbuf[rdisp[rp]], R[p], nR[p]); // sum receive buffer into values
	}
...
/** @brief Perform a sparse sum reduction (or mapped reduction) using a ring-like non-blocking communication scheme
...
/** @brief Perform a sparse sum reduction (or mapped reduction) using a ring-like non-blocking no-empty communication scheme
    @param R pointer to receiving maps
    @param nR array of number of elements in each receiving map

...
*/
int
mpi_stbmm(double **V, int64_t nrow, int m, int m_rowwise, Block *tpltzblocks, int nb_blocks_local, int nb_blocks_all,
          int64_t idp, int local_V_size, Flag flag_stgy, MPI_Comm comm) {
#else // for sequential use only
    int mpi_stbmm(double **V, int64_t nrow, int m, int m_rowwise, Block *tpltzblocks, int nb_blocks_local,
                  int nb_blocks_all, int64_t idp, int local_V_size, Flag flag_stgy) {
#endif
        ...
        int status_params = get_overlapping_blocks_params(nb_blocks_local, tpltzblocks, local_V_size, nrow, idp,
                                                          &idpnew, &local_V_size_new, nnew, &idv0, &idvn);
        ...
        if (PRINT_RANK == 0 && VERBOSE > 2) { // print on screen news parameters definition if VERBOSE
        ...
        if (ncol_rank == 1) // Empty process not allowed
            ...
            nb_blocks_rank = (ncol_rank - 2) * nb_blocks_local + (nb_blocks_local - idv0) +
        ...
        int idvp0 =
                idpnew % nrow - tpltzblocks[idv0].idv; // index of the first element of the process in the first block
        ...
        idvpn = tpltzblocks[idvn].idv + nnew[idvn] - 1 - (idpnew + local_V_size_new - 1) % nrow;
        ...
        if (flag_optimlambda == 1) {
            ...
            LambdaOut = (double *) calloc((tpltzblocks[idv0].lambda + tpltzblocks[idvn].lambda) * m_rowwise,
                                          sizeof(double));
            lambdaOut_offset = tpltzblocks[idv0].lambda * m_rowwise;
            lambdaIn_offset = tpltzblocks[idv0].lambda * m_rowwise;
            lambdaOut_size = (tpltzblocks[idv0].lambda + tpltzblocks[idvn].lambda) * m_rowwise;
            lambdaIn_size = (tpltzblocks[idv0].lambda + tpltzblocks[idvn].lambda) * m_rowwise;
        }
        ...
        if (offsetn != 0) {
            for (j = 0; j < m_rowwise; j++)
                for (i = 0; i < toSendRight; i++)
                    LambdaOut[i + j * toSendRight + lambdaOut_offset] = (*V)[i + j * n_rowwise + local_V_size -
                                                                             toSendRight];
        ...
        if (flag_blockingcomm == 1) {
            ...
            MPI_Sendrecv(LambdaOut, toSendLeft * m_rowwise, MPI_DOUBLE, left, MPI_USER_TAG,
                         (LambdaIn + lambdaIn_offset), offsetn * m_rowwise, MPI_DOUBLE, right, MPI_USER_TAG, comm,
                         &status);
            MPI_Sendrecv((LambdaOut + lambdaOut_offset), toSendRight * m_rowwise, MPI_DOUBLE, right, MPI_USER_TAG,
                         LambdaIn, offset0 * m_rowwise, MPI_DOUBLE, left, MPI_USER_TAG, comm, &status);
        } else {
            // to the Left
            MPI_Irecv((LambdaIn + lambdaIn_offset), offsetn * m_rowwise, MPI_DOUBLE, right, MPI_USER_TAG, comm,
                      &requestLeft_r);
            ...
            MPI_Isend((LambdaOut + lambdaOut_offset), toSendRight * m_rowwise, MPI_DOUBLE, right, MPI_USER_TAG, comm,
                      &requestRight_s);
        }
        ...
        if (nb_blocks_rank == 1) { // only one block
            ...
            v0rank_size = tpltzblocks[idv0].idv + nnew[idv0] - idpnew % nrow + offset0;
            vnrank_size = ((idpnew + local_V_size_new - 1) % nrow + 1) - tpltzblocks[idvn].idv + offsetn;
        }
...
#pragma omp parallel for // num_threads(NB_OMPTHREADS_STBMM)
                            for (i = 0; i < offsetn; i++) {
                                V1block[vblock_size - offsetn + i + j * vblock_size] = LambdaIn[i + lambdaIn_offset +
                                                                                                j * offsetn];
                            }
                        }
                    }
                    ...
                    tpltz_init(vblock_size, tpltzblocks[id].lambda, &nfft, &blocksize, &T_fft,
                               (tpltzblocks[id].T_block), &V_fft, &V_rfft, &plan_f, &plan_b, flag_stgy);

                    // Toeplitz computation
                    if (PRINT_RANK == 0 && VERBOSE > 2)
                        fprintf(file, "[%d] Before stmm_main call : nfft = %d, blocksize = %d\n", rank, nfft,
                                blocksize);
                    stmm_main(&V1block, vblock_size, m_rowwise, 0, m_rowwise * vblock_size, (tpltzblocks[id].T_block),
                              T_fft, tpltzblocks[id].lambda, V_fft, V_rfft, plan_f, plan_b, blocksize, nfft, flag_stgy);
...
#pragma omp parallel for // num_threads(NB_OMPTHREADS_STBMM)
                    ...
                    // init Toeplitz arrays
                    tpltz_init(nnew[id], tpltzblocks[id].lambda, &nfft, &blocksize, &T_fft, (tpltzblocks[id].T_block),
...
#pragma omp parallel for // num_threads(NB_OMPTHREADS_STBMM)
                    ...
                    idv1 = (tpltzblocks[id].idv) - idp % nrow - vShft + offset0 + nrow * ((iblock / nb_blocks_local));
                    idv2 = (tpltzblocks[id].idv) - (idpnew) % nrow + vShft + nrow * ((iblock / nb_blocks_local));
...
#pragma omp parallel for // num_threads(NB_OMPTHREADS_STBMM)
                    ...
                    stmm_main(&V1block, vblock_size, m_rowwise, 0, vblock_size * m_rowwise, (tpltzblocks[id].T_block),
                              T_fft, tpltzblocks[id].lambda, V_fft, V_rfft, plan_f, plan_b, blocksize, nfft, flag_stgy);
...
#pragma omp parallel for // num_threads(NB_OMPTHREADS_STBMM)
                    ...
                    break;
                } // error  //we can put the generic case here instead of between first and last cases
#endif
                //-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
            } // end of if(nnew[id]>0)
...
*/
    int get_overlapping_blocks_params(int nbloc, Block *tpltzblocks, int local_V_size, int64_t nrow, int64_t idp,
                                      int64_t *idpnew, int *local_V_size_new, int *nnew, int *ifirstBlock,
                                      int *ilastBlock) {
        ...
        if (nfullcol_data > 0) {
            ...
            for (ib = 0; ib < nbloc; ib++) {
                if (tpltzblocks[ib].idv < nrow) {
                    nnew[ib] = min(tpltzblocks[ib].n, nrow - tpltzblocks[ib].idv); // block used for the product
                    nblockOK++;
                }
            }
            ...
            if (firstrow < lastrow) { // just one column partially observed
                ...
                for (ib = 0; ib < nbloc; ib++) {
                    if ((tpltzblocks[ib].idv + tpltzblocks[ib].n > firstrow) && (tpltzblocks[ib].idv <
                                                                                 nrow)) {
                        // intersects first partial column
                        nnew[ib] = min(tpltzblocks[ib].n, nrow - tpltzblocks[ib].idv); // block used for the product
                        nblockOK++;
                    }

                    if ((tpltzblocks[ib].idv < lastrow + 1) && (tpltzblocks[ib].idv + tpltzblocks[ib].n >
                                                                0)) {
                        // intersects second partial column
                        nnew[ib] = min(tpltzblocks[ib].n, nrow - tpltzblocks[ib].idv); // block used for the product
        ...
        if (nblockOK == 0)
            return (0); // no blocks overlapping with the data
        ...
        for (*ifirstBlock = -1; *ifirstBlock == -1;) {
            ...
            if (ib < nbloc && tpltzblocks[ib].idv <= idptmp % nrow) {
                ...
                *ifirstBlock = ib;
                //   int64_t extrabegining = tpltzblocks[ib].idv-idp%nrow;  //note I put int64 just to be sure. Never used
        ...
        for (*ilastBlock = -1; *ilastBlock == -1;) {
            for (ib = nbloc - 1; ib >= 0; ib--) {
                if (nnew[ib] != 0 && tpltzblocks[ib].idv <= idptmp % nrow)
                    break;
            }
            ...
            if (ib >= 0 && idptmp % nrow < tpltzblocks[ib].idv + nnew[ib]) {
                ...
                *ilastBlock = ib;
                // int64_t extraend = (local_V_size-1+idp)%nrow+1-(tpltzblocks[ib].idv+nnew[ib]);  //note I put int64 just to be sure
                //*local_V_size_new = (local_V_size+idp)%nrow-(idv[*ilastBlock]+nnew[*ilastBlock]);
                // idv[*ilastBlock]+nnew[*ilastBlock]-(*idpnew);
                //*local_V_size_new = local_V_size-(*idpnew)+idp-extraend;  //compute twice ... ? remove this one

                int idvlastcolumn = idptmp / nrow;
                *local_V_size_new = tpltzblocks[ib].idv + nnew[ib] + idvlastcolumn * nrow - (*idpnew);
            } else {
                idptmp = idptmp - (idptmp % nrow) - 1; //(int) idptmp - (idptmp%nrow)-1;

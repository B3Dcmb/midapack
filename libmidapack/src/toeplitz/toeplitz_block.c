...
int mpi_stbmm(double **V, int64_t nrow, int m, int m_rowwise, Block *tpltzblocks, int nb_blocks_local, int nb_blocks_all, int64_t idp, int local_V_size, Flag flag_stgy)
{
#endif
	...
	if (PRINT_RANK == 0 && VERBOSE > 2)
	{ // print on screen news parameters definition if VERBOSE
	...
	int idvp0 = idpnew % nrow - tpltzblocks[idv0].idv; // index of the first element of the process in the first block
	...
	{
		LambdaOut = (double *)calloc((tpltzblocks[idv0].lambda + tpltzblocks[idvn].lambda) * m_rowwise, sizeof(double));
...
#pragma omp parallel for // num_threads(NB_OMPTHREADS_STBMM)
				...
				tpltz_init(vblock_size, tpltzblocks[id].lambda, &nfft, &blocksize, &T_fft, (tpltzblocks[id].T_block), &V_fft, &V_rfft, &plan_f, &plan_b, flag_stgy);
...
#pragma omp parallel for // num_threads(NB_OMPTHREADS_STBMM)
			...
			else if (iblock != idv0 && iblock != idv0 + nb_blocks_rank - 1)
			{
				...
				idv1 = (tpltzblocks[id].idv) - idp % nrow - vShft + offset0 + nrow * ((iblock / nb_blocks_local)); // no need
...
#pragma omp parallel for // num_threads(NB_OMPTHREADS_STBMM)
				...
				stmm_main(&V1block, vblock_size, m_rowwise, 0, vblock_size * m_rowwise, (tpltzblocks[id].T_block), T_fft, tpltzblocks[id].lambda, V_fft, V_rfft, plan_f, plan_b, blocksize, nfft, flag_stgy);
...
int get_overlapping_blocks_params(int nbloc, Block *tpltzblocks, int local_V_size, int64_t nrow, int64_t idp, int64_t *idpnew, int *local_V_size_new, int *nnew, int *ifirstBlock, int *ilastBlock)
{
	...
	{ // no full column observed

		firstrow = idp % nrow;
		lastrow = (idp + local_V_size - 1) % nrow;

		if (firstrow < lastrow)
		{ // just one column partially observed
		...
		{ // two columns partially observed

			for (ib = 0; ib < nbloc; ib++)
			{
				if ((tpltzblocks[ib].idv + tpltzblocks[ib].n > firstrow) && (tpltzblocks[ib].idv < nrow))
				{																   // intersects first partial column
					nnew[ib] = min(tpltzblocks[ib].n, nrow - tpltzblocks[ib].idv); // block used for the product
					nblockOK++;
				}

				if ((tpltzblocks[ib].idv < lastrow + 1) && (tpltzblocks[ib].idv + tpltzblocks[ib].n > 0))

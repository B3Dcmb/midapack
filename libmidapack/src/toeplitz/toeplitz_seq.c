...
//=========================================================================
/// Perform the product of a Toeplitz matrix by a general matrix using the
/// sliding window algorithm.
/** @ingroup group11
    This is a simplifed call of the sequential product including initialization
   and cleaning. This use the flag parameters to set the comutational options.
   ...
   row \param lambda number of non-zero in the first row of the Toeplitz and
   size of T \param flag_stgy flag strategy for the product computation
*/
...
/** @ingroup group12
   ...
   parameters (idv, n, T_block, lambda) : \param nb_blocks number of Toeplitz
   blocks as stored in T \param idp global index of the first element of the
   local part of V \param local_V_size a number of all elements in local V
...
/// Performs the multiplication of a symmetric, Toeplitz block-diagonal matrix
/// with gaps, T, by an arbitrary matrix, V, distributed over processes.
...
/** @ingroup group11
   ...
   computation cost and skip the computations of the values on the defined gaps.
    ...
    The parameters are :
    \param V \b [input] distributed data matrix (with the convention

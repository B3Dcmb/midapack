...
int defineTpltz(Tpltz *Nm1, int64_t nrow, int m_cw, int m_rw,
...
int defineBlocks_avg(Block *tpltzblocks, double *T, int nb_blocks_loc,
                     int n_block_avg, int lambda_block_avg, int64_t id0) {

    int i;


    for (i = 0; i < nb_blocks_loc; i++) tpltzblocks[i].n = n_block_avg;
    ...
    for (i = 0; i < nb_blocks_loc; i++) { tpltzblocks[i].T_block = (T); }
...
int createRandomT(double *T, int Tsize) {
    ...
    for (i = 0; i < Tsize; i++) T[i] = rand() / ((double) RAND_MAX);
...
int createTbasic1(double *T, int Tsize) {
    ...
    // input matrix definition of T
    for (i = 0; i < Tsize; i++) T[i] = 1.0 + rand() / ((double) RAND_MAX);

...
// int main_nontrue(int argc, char *argv[]){
...
//  fclose(out);
  ...
  printf("%d 000 -- ncom_A : %d\n", rank, ncom_A);
  double *vA_received = (double *) malloc(nA * sizeof(double));
  shared = m2m(vcom_A, com_A, ncom_A, vA_received, A, nA);

  printf("%d --- ##### Data received : \n", rank); fflush(stdout);
  printf("%d -- ncom_A : %d\n", rank, ncom_A);
  if (ncom_A > 0){
      printf("%d -- indices_received : %d", rank, com_A[0]);
      for (i=1; i<ncom_A; i++){
          printf("- %d -", com_A[i]);
      }
      printf("\n");
  ...
  fflush(stdout);
  printf("%d --- #####2 Data received : \n", rank); fflush(stdout);
  printf("%d -- nA : %d\n", rank, nA);
  if (nA > 0){
      printf("%d -- indices_received A : %d", rank, A[0]);
      for (i=1; i<nA; i++){
          printf("- %d -", A[i]);
      }
      printf("\n");
      printf("%d -- vA : %f", rank, vA_received[0]);
      for (i=1; i<nA; i++){
          printf("- %f -", vA_received[i]);
      }
      printf("\n");
  }
  fflush(stdout);
...
//  fclose(out);
  ...
  MPI_Barrier(MPI_COMM_WORLD);
  t0=MPI_Wtime();
  truebutterfly_init(A, nA, R, nR, S, nS, &com_A, &ncom_A, steps, MPI_COMM_WORLD/*, fn*/);
  t1=MPI_Wtime();
...
//  fclose(out);
  ...
  MPI_Barrier(MPI_COMM_WORLD);
  t0=MPI_Wtime();
  truebutterfly_reduce(R, nR, nRmax, S, nS, nSmax, vcom_A, steps, MPI_COMM_WORLD);
  t1=MPI_Wtime();
  printf("%d Reduce finished ! \n", rank); fflush(stdout);

...
/// sequentially or using MPI. The complexity is hidden in the input structure,
/// which needs to be defined by a user.
/** @ingroup wizard
 **/
...
int stbmmProd(Tpltz *Nm1, double *V) {
    ...
    mpi_stbmm(&V, Nm1->nrow, Nm1->m_cw, Nm1->m_rw, Nm1->tpltzblocks,
              Nm1->nb_blocks_loc, Nm1->nb_blocks_tot, Nm1->idp,
              Nm1->local_V_size, Nm1->flag_stgy, Nm1->comm);

#else

    stbmm(&V, Nm1->nrow, Nm1->m_cw, Nm1->m_rw, Nm1->tpltzblocks,
          Nm1->nb_blocks_loc, Nm1->idp, Nm1->local_V_size, Nm1->flag_stgy);
...
int gstbmmProd(Tpltz *Nm1, double *V, Gap *Gaps) {
    ...
    mpi_gstbmm(&V, Nm1->nrow, Nm1->m_cw, Nm1->m_rw, Nm1->tpltzblocks,
               Nm1->nb_blocks_loc, Nm1->nb_blocks_tot, Nm1->idp,
               Nm1->local_V_size, Gaps->id0gap, Gaps->lgap, Gaps->ngap,
               Nm1->flag_stgy, Nm1->comm);

#else

    gstbmm(&V, Nm1->nrow, Nm1->m_cw, Nm1->m_rw, Nm1->tpltzblocks,
           Nm1->nb_blocks_loc, Nm1->idp, Nm1->local_V_size, Gaps->id0gap,
           Gaps->lgap, Gaps->ngap, Nm1->flag_stgy);

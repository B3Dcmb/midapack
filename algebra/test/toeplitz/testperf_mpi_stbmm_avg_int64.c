...
//parameters
  int64_t nrow = (int64_t) pow(2,32); //31); //28);//30); //28);//pow(2,25);//20);//you need to choose M so M/(nb_procs) = int to havec the same size everywhere

  int local_V_size = nrow/size;
  int64_t id0 = (int64_t) rank*local_V_size;
  ...
  int lambda_block_avg = pow(2,14);//n_block_avg/pow(2,10);//pow(2,3);//3;
  Flag flag_stgy;
...
//print parameters:
  ...
  for( i=0; i<nb_blocks_loc; i++) 
    printf("[rank %d] tpltzblocks[i].idv=%ld \t", rank, tpltzblocks[i].idv);
  printf("[rank %d] \n", rank);

...
@note 
@note This program is free software; you can redistribute it and/or modify it under the terms of the GNU Lesser General Public License as published by the Free Software Foundation; either version 3 of the License, or
@note (at your option) any later version. This program is distributed in the hope that it will be useful, but WITHOUT ANY WARRANTY; without even the implied warranty of
@note MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE. See the GNU Lesser General Public License for more details.
@note 
@note You should have received a copy of the GNU Lesser General Public License along with this program; if not, see http://www.gnu.org/licenses/lgpl.html
...
int stmm_cblas(int n_loc, int m_loc, double *T2_loc, double *V, double *TV2) {
  ...
  cblas_dgemm (CblasColMajor, CblasNoTrans,CblasNoTrans, n_loc, m_loc, n_loc, 1, T2_loc, n_loc, (V), n_loc, 1, TV2, n_loc);
...
int build_full_Toeplitz(int n_loc, double *T_loc, int lambda_loc, double *T2_loc) {
  ...
  int i,j;
      ...
      for (j=0;j<n_loc;j++){ // Full Toeplitz matrix needed for cblas computation
        for (i=0;i<lambda_loc;i++){
          if (j-i>=0)
            T2_loc[j*n_loc+j-i] = T_loc[i];
          if (j+i<n_loc)
            T2_loc[j*n_loc+j+i] = T_loc[i]; }}

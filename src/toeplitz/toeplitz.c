...
/** @ingroup group21
    The optimal block size is computed as the minimum power of two above 3*lambda, i.e.
    the smallest value equal to 2^x, where x is an integer, and above 3*lambda.
    If bs_flag is set to one, a different formula is used to compute the optimal block size
    (see MADmap: A MASSIVELY PARALLEL MAXIMUM LIKELIHOOD COSMIC MICROWAVE BACKGROUND
    MAP-MAKER, C. M. Cantalupo, J. D. Borrill, A. H. Jaffe, T. S. Kisner, and R. Stompor,
    The Astrophysical Journal Supplement Series, 187:212–227, 2010 March).
    To avoid using block size much bigger than the matrix, the block size is set to 3*lambda
    when his previous computed size is bigger than the matrix size n. This case append mostly
    for small matrix compared to his bandwith.
    \param n matrix row dimension
    \param lambda half bandwidth of the Toeplitz matrix
...
/** @ingroup group11
    ...
    \param blocksize optimal block size used in the sliding window algorithm
    to compute an optimize value)
...
/** @ingroup group21
    ...
    \param nfft maximum number of FFTs you want to compute at the same time
    \param fft_size effective FFT size for the general matrix V (usually equal to blocksize)
    \param V_fft complex array used for FFTs
    \param V_rfft real array used for FFTs
    \param plan_f fftw plan forward (r2c)
    \param plan_b fftw plan backward (c2r)
...
/** @ingroup group21
    Builds the circulant matrix T_circ from T and initilizes its fftw arrays and plans. Use
    tpltz_cleanup afterwards. \sa tpltz_cleanup
    \param T Toeplitz matrix.
    \param fft_size effective FFT size for the circulant matrix (usually equal to blocksize)
    \param lambda Toeplitz band width.
    \param T_fft complex array used for FFTs.
 */
...
/** @ingroup group21
    This routine multiplies a circulant matrix, represented by C_fft, by a general matrix V,
    and stores the output as a matrix CV. In addition the routine requires two workspace
    objects, V_fft and V_rfft, to be allocated prior to a call to it as well as two fftw plans:
    one forward (plan_f_V), and one backward (plan_b_TV). The sizes of the input general matrix
    V and the ouput CV are given by blocksize rows and m columns. They are stored as a vector
    in the column-wise order. The circulant matrix, which is assumed to be band-diagonal with
    a band-width lambda, is represented by a Fourier transform with its coefficients stored in
...
/** @ingroup group21
    ...
    how many spurious values are computed extra due to overlaps between the blocks.
    Use flag_offset=0 for "classic" algorithm and flag_offset=1 to put an offset to avoid the
    first and last lambdas terms. Usefull when a reshaping was done before with optimal column

...
class GapFillInfo {
  public:
    ...
    void store_block_time(double milliseconds) {
        block_times[current_block] = milliseconds * 0.001;
    }
    void store_pcg_time(double milliseconds) {
        pcg_times[current_block] = milliseconds * 0.001;
    }
    ...
    std::vector<double>
        valid_fracs; // proportion of valid samples in each block
...
class GapFillRecap {
  ...
  private:
    ...
    friend std::ostream &operator<<(std::ostream &out,
                                    GapFillRecap const &recap);
};

int find_valid_samples(Gap *gaps, size_t id0, std::vector<uint8_t> &valid);

void sim_constrained_noise_block(GapFillInfo &gfi, Tpltz *N_block,
                                 Tpltz *Nm1_block, double *noise, Gap *gaps,
                                 int w0, uint64_t realization, uint64_t detindx,
                                 uint64_t obsindx, uint64_t telescope,
                                 double sample_rate);

void sim_constrained_noise(GapFillInfo &gfi, Tpltz *N, Tpltz *Nm1,
                           double *noise, Gap *gaps, uint64_t realization,
...
void flagged_running_average(int samples, const double *buf, double *baseline,
                             const uint8_t *valid, int bandwidth);

void remove_baseline(int samples, double *buf, double *baseline,
                     const uint8_t *valid, int bandwidth, bool rm);

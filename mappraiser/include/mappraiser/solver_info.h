...
struct solverinfo_t {
    ...
    double abs_res_reduct; // absolute residual norm (tolerance) (stop if |r_i|
                           // <= epsilon)
    double rel_res_reduct; // relative residual norm (stop if |r_i| <= epsilon *
                           // |r_0|)
    double rel_res_growth; // relative residual growth (divergence) (stop if
                           // |r_i| >= epsilon * |r_0|)
    ...
    bool has_diverged;  // did the iteration diverge (based on a stopping
                        // criterion)
    ...
    double *res_hist; // complete iteration history (only used if store_hist is
                      // true)
};

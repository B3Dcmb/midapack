...
class RunningSum {
  public:
    ...
    void process_index(int nval, int idx, const ValueType *values,
  ...
  private:
    void internal_process_index(int nval, int idx, const ValueType *values,
                                const ValidType *valid);
    ...
    int m_first_elem; // smallest index currently part of the sum (whether valid
                      // or not)
    int m_last_elem;  // largest index currently part of the sum (whether valid
                      // or not)
};
template <typename ValueType, typename ValidType>
void RunningSum<ValueType, ValidType>::print(std::ostream &out) const {
    out << "count " << m_count << " (first " << m_first_elem << ", last "
        << m_last_elem << ")";
}

template <typename ValueType, typename ValidType>
RunningSum<ValueType, ValidType>::RunningSum(int w0)
    : m_acc(0), m_count(0), m_w0(w0), m_w(w0), m_threshold_low(0.2),
      m_first_elem(1), m_last_elem(0) {}

template <typename ValueType, typename ValidType>
__attribute__((unused))
RunningSum<ValueType, ValidType>::RunningSum(int w0, double thresh_low)
    : m_acc(0), m_count(0), m_w0(w0), m_w(w0), m_threshold_low(thresh_low),
      m_first_elem(1), m_last_elem(0) {}

template <typename ValueType, typename ValidType>
void RunningSum<ValueType, ValidType>::process_index(int nval, int idx,
                                                     const ValueType *values,
                                                     const ValidType *valid) {
    // process current index with current window
    // the current window depends on what happened while processing previous
    // indices
    ...
    while (m_count < nbr_samples_ref * m_threshold_low) {
        ...
        // if we have some samples and current window is larger than 4 times w0,
        // stop
        if (m_count > 0 && m_w > 4 * m_w0)
            break;
    }
...
template <typename ValueType, typename ValidType>
void RunningSum<ValueType, ValidType>::internal_process_index(
    int nval, int idx, const ValueType *values, const ValidType *valid) {
...
template <typename ValueType, typename ValidType>
void RunningSum<ValueType, ValidType>::add(int pos, ValueType value,
                                           ValidType valid) {
    if (valid) {
        ++m_count;
        m_acc += value;
...
template <typename ValueType, typename ValidType>
void RunningSum<ValueType, ValidType>::rmv(int pos, ValueType value,
                                           ValidType valid) {
    if (valid) {
        --m_count;
        m_acc -= value;

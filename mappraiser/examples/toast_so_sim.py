...
from TOAST_interface import mappraiser, MyPerturbHWP
...
def parse_config(operators, templates, comm):
    ...
    parser.add_argument(
        ...
        default=False,
        action="store_true",
        help="If True, use the perturbed HWP angle in the pointing matrix.",
    )
...
def reduce_data(job, args, data):
    ...
    ops.mappraiser.stokes_weights = ops.weights_mappraiser
...
def main():
    ...
    operators.append(MyPerturbHWP(name="perturb_hwp"))
    operators.append(toast.ops.StokesWeights(name="weights_mappraiser", mode="IQU"))

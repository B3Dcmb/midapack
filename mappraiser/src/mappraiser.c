...
void MLmap(MPI_Comm comm, char *outpath, char *ref, int solver, int precond, int Z_2lvl, int pointing_commflag, double tol, int maxiter, int enlFac, int ortho_alg, int bs_red, int nside, void *data_size_proc, int nb_blocks_loc, void *local_blocks_sizes, int Nnz, void *pix, void *pixweights, void *signal, double *noise, int lambda, double *invtt, int bool_apply_filter, int domain_PCG_computation, int lmax_Wiener_Filter, char *c_ell_path, int number_correlations)
{
    ...
    // reduce the size of map-domain objects in case the trash_pix flag is raised
    ...
    initialize_PCG_var_struct(&PCG_variable, local_map_pix);
    ...
    if (solver == 0)
    {
        ...
        PCG_GLS_true(outpath, ref, &A, &Nm1, &PCG_variable, signal, noise, cond, lhits, tol, maxiter, precond, Z_2lvl, is_pixel_scheme_ring, nside, &Harmonic_sup, lmax_Wiener_Filter, c_ell_path, number_correlations);
    }
    else if (solver == 1)
    {
#ifdef WITH_ECG
...
int x2map_pol(double *mapI, double *mapQ, double *mapU, double *Cond, int *hits, int npix, double *x, int *lstid,

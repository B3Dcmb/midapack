...
int defineBlocks_avg(Block *tpltzblocks, double *T, int nb_blocks_loc, void *local_blocks_sizes, int lambda_block_avg,
    ...
    for (i = 0; i < nb_blocks_loc; i++) tpltzblocks[i].n = ((int *) local_blocks_sizes)[i];

    for (i = 0; i < nb_blocks_loc; i++) tpltzblocks[i].lambda = lambda_block_avg;

    // tpltzblocks[0].idv = (int64_t) (id0/n_block_avg) * n_block_avg ;
    tpltzblocks[0].idv = id0;
    for (i = 1; i < nb_blocks_loc; i++) tpltzblocks[i].idv = (int64_t) tpltzblocks[i - 1].idv + tpltzblocks[i - 1].n;

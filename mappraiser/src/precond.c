...
int precondblockjacobilike(Mat *A, Tpltz *Nm1, double *vpixBlock,
    ...
    int off = A->ignore_extra ? A->trash_pix : 0;
...
void build_BJinv(Mat *A, Tpltz *Nm1, Mat *BJ_inv, double **cond, int **lhits,
    ...
    if (nd > 0) {
        ...
        MatInit(A, A->m, nnz, A->indices, A->values, A->flags, MPI_COMM_WORLD);
    ...
    int off_extra = A->ignore_extra ? A->trash_pix * nnz : 0;
    for (int i = 0; i < n; i++) {
        for (int j = 0; j < nnz; j++) {
            indices_new[i * nnz + j] =

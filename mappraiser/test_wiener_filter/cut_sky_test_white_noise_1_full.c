...
int main(int argc, char** argv){
    ...
    char *covariance_matrix_path = "/global/homes/m/mag/midapack/mappraiser/src/test_wiener_filter/cls_limited_128.fits";
    char *path_mask = "";
    int rank, size;
    ...
    int *mask_binary= (int *)malloc(npix*sizeof(int));
    ...
    init_files_struct_WF(&(S2HAT_params.Files_WF_struct), nside, lmax, covariance_matrix_path, number_correlations);
    ...
    inverse_covariance_matrix = (double **)malloc((lmax+1)*(lmax+1)*sizeof(double *));
    for(ell_value=0; ell_value<(lmax+1)*(lmax+1); ell_value++){
    ...
    int dimensions_huge_covariance_matrix = nstokes*(lmax+1);
    char cholesky_part = 'L';
    printf("%d --- Getting Cholesky decomposition of inverse ! \n");
    dpotrf_(&cholesky_part, &dimensions_huge_covariance_matrix, covariance_matrix_1d, &nstokes, &info);
    ...
    if (rank == 0){
        ...
        for(i=0;i<nstokes; i++)
            ...
            sprintf(filename_save, "/pscratch/sd/m/mag/WF_work/map_files_test_WF/cutsky_mapfile_whiten_1a_%d.fits", i);

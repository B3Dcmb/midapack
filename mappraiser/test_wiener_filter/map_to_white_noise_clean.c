...
int main_clean_ver_nside_64_polar(int argc, char** argv){
// int main(int argc, char** argv){
    ...
    char *c_ell_path = "/global/homes/m/mag/midapack/mappraiser/src/test_wiener_filter/cls_limited_128_woTTTE_v2.fits";
    int rank, size;
    ...
    char save_filename[100] = "/pscratch/sd/m/mag/WF_work/map_files_test_WF/cell_prewitened_newver_vtuned_1b.fits";
    ...
    for(ell_value=0; ell_value<lmax; ell_value++){
        ...
        inverse_covariance_matrix[ell_value][0] = 1/sqrt(covariance_matrix[ell_value][0])/sqrt(normalization_factor); //*(2*ell_value + 1));
        inverse_covariance_matrix[ell_value][1] = 0;
        inverse_covariance_matrix[ell_value][2] = 0;
        inverse_covariance_matrix[ell_value][3] = 1/sqrt(covariance_matrix[ell_value][3])/sqrt(normalization_factor); //*(2*ell_value + 1));
        // if (nstokes == 3){
        //     inverse_covariance_matrix[ell_value][6] = 0;
        //     inverse_covariance_matrix[ell_value][7] = 0;
        // }
...
int main(int argc, char** argv){
    ...
    if (rank == 0){
        ...
        for(i=0;i<nstokes; i++)
            ...
            // sprintf(filename_save, "/pscratch/sd/m/mag/WF_work/map_files_test_WF/mapfile_filtered_newver_woTTTE_v2_1_%d.fits", i);
            // sprintf(filename_save, "/pscratch/sd/m/mag/WF_work/map_files_test_WF/mapfile_filtered_newver_woTTTE_vtuned_5b_%d.fits", i);
            sprintf(filename_save, "/pscratch/sd/m/mag/WF_work/map_files_test_WF/mapfile_filtered_newver_vtuned_1_%d.fits", i);
            printf("--- %d -- Saving Stokes components maps \n", i); fflush(stdout);
            for(j=0; j<npix; j++)
                full_sky_map_1_Stokes[j] = full_sky_map_2[j + i*npix];
            write_healpix_map(full_sky_map_1_Stokes, nside, filename_save, 0, "C");
